...
const inputs = [
  ...document.querySelectorAll(".list-of-products li ul li input"),
];
  ...
  input.addEventListener("click", () => {
    if (input.checked === true) {
      ...
      mainList.insertAdjacentHTML(
        "beforeend",
        `<li class='item' data-number="${inputClosest.attributes[0].value}"><i class="fas fa-times"></i>${itemValue}</li>`
      );
      markAndDelete();
    } else {
...
const addNewCategory = function () {
  ...
  if (newCategory) {
    ...
    newLiCategory.insertAdjacentHTML(
      "beforeend",
      `<ul>
...
categoryField.addEventListener("keydown", () =>
  errorContainer.classList.remove("active")
);
